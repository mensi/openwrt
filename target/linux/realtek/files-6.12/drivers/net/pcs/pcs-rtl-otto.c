...
static const struct rtpcs_sds_config rtpcs_930x_sds_cfg_usxgmii_qx_even[] = {
...
static const struct rtpcs_sds_config rtpcs_930x_sds_cfg_usxgmii_qx_odd[] = {
	{0x06, 0x00, 0x0000}, {0x06, 0x0D, 0x0F00}, {0x06, 0x0E, 0x055A}, {0x06, 0x1D, 0x0600},
	{0x07, 0x10, 0x6003}, {0x06, 0x13, 0x68C1}, {0x06, 0x14, 0xF021}, {0x07, 0x06, 0x1401},
	{0x21, 0x03, 0x8206}, {0x21, 0x05, 0x40B0}, {0x21, 0x06, 0x0010}, {0x21, 0x07, 0xF09F},
	{0x21, 0x0A, 0x0003}, {0x21, 0x0B, 0x0005}, {0x21, 0x0C, 0x0007}, {0x21, 0x0D, 0x6009},
				 ...
				 enum rtpcs_sds_mode hw_mode)
{
	...
	if (hw_mode == RTPCS_SDS_MODE_USXGMII_10GQXGMII) {
		/*
		 * USXGMII QXGMII post-patch configuration for RTL8224 PHY.
		 * The patch tables above write initial values, but the SDK
		 * then overrides specific fields via _dal_longan_sds_usxgmii_cfg_set()
		 * using sds_usxgmii_cfg_rtl8224:
		 *   nway_enable=1, opcode=0xaa, am_period=0x5078,
		 *   am_markers=all zero, an_table=1, sync_bit=0x1
		 *
		 * Without these overrides, the SoC-side QXGMII link does not
		 * establish correctly with external switches.
		 */

		/* nway enable: set bits[3:0] of page 0x7, reg 0x11 */
		rtpcs_sds_write_bits(sds, 0x7, 0x11, 0, 0, 1);
		rtpcs_sds_write_bits(sds, 0x7, 0x11, 1, 1, 1);
		rtpcs_sds_write_bits(sds, 0x7, 0x11, 2, 2, 1);
		rtpcs_sds_write_bits(sds, 0x7, 0x11, 3, 3, 1);

		/* am_period = 0x5078 */
		rtpcs_sds_write_bits(sds, 0x6, 0x12, 15, 0, 0x5078);

		/* am_markers = all zero (overrides non-zero values from patch table) */
		rtpcs_sds_write_bits(sds, 0x6, 0x13, 7,  0, 0);
		rtpcs_sds_write_bits(sds, 0x6, 0x13, 15, 8, 0);
		rtpcs_sds_write_bits(sds, 0x6, 0x14, 7,  0, 0);
		rtpcs_sds_write_bits(sds, 0x6, 0x14, 15, 8, 0);
		rtpcs_sds_write_bits(sds, 0x6, 0x15, 7,  0, 0);
		rtpcs_sds_write_bits(sds, 0x6, 0x15, 15, 8, 0);
		rtpcs_sds_write_bits(sds, 0x6, 0x16, 7,  0, 0);
		rtpcs_sds_write_bits(sds, 0x6, 0x16, 15, 8, 0);
		rtpcs_sds_write_bits(sds, 0x6, 0x17, 7,  0, 0);
		rtpcs_sds_write_bits(sds, 0x6, 0x17, 15, 8, 0);
		rtpcs_sds_write_bits(sds, 0x6, 0x18, 7,  0, 0);
		rtpcs_sds_write_bits(sds, 0x6, 0x18, 15, 8, 0);

		/* opcode = 0xaa (overrides 0x03 from patch table) */
		rtpcs_sds_write_bits(sds, 0x7, 0x10, 7, 0, 0xaa);

		/* an_table = 1 */
		rtpcs_sds_write_bits(sds, 0x6, 0xe, 10, 10, 1);

		/* sync_bit = 0x1 */
		rtpcs_sds_write_bits(sds, 0x6, 0x1d, 11, 10, 0x1);
	}
}
